...
const StudentProfile = () => {
  ...
  if (!isChanged) {
    setMessage({ type: "error", text: "No changes have been made yet." });
    return;
  }
    try {
  ...
  if (!profile)
    return (
      <p className="text-center text-lg text-red-500">No profile data found.</p>
    );
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-xl shadow-lg mt-10">
      ...
      {isEditing ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <strong>Class:</strong> {profile.class_grade}
          </div>
          <div>
            <strong>Date of Birth:</strong> {profile.date_of_birth?.split("-").reverse().join("-")}
          </div>
          <div>
            <strong>Admission Date:</strong> {profile.admission_date?.split("-").reverse().join("-")}
          </div>
          <div>
            <strong>Status:</strong> {profile.status}
          </div>
          <div>
            <strong>Assigned Teacher:</strong> {profile.teacher_name}
          </div>
        </div>
      )}
